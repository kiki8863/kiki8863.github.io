...
PAGE_SELLER.start = function(pageSub, pagData){
	...
	*/
	...
	PAGE_SELLER.DATA.PAGE_MEMU = pageMenu;
	var defaultHash = '';
	if(pageSub != 'seller_home'){
		defaultHash = '#!action=seller&group='+pageSub+'&id='+pageMenu;	
	}
...
PAGE_SELLER.setHashCheck = function(hashPrams){
...
PAGE_SELLER.setPagination = function(saveData, getIsChangePage){
	...
	if($paginationEle.length){
		...
		if(isDefined(PAGE_SELLER.DATA.pagination) == true && isDefined(PAGE_SELLER.DATA.paginationEle) == true && isChangePage == false){
			console.log('has p');
	...
	var successFunGetMovieContentsList = function(data){
		...
		if(isDefined(data.seller_page_info)){
			...
			if(isPageShow == true){
				...
				if(isDefined(data.seller_page_info.is_force_page)){
					if(data.seller_page_info.is_force_page == true || data.seller_page_info.is_force_page == 'true'){
						isNewPage = true;
					}
				}
		...
		if(isPageShow == true){
			...
			if(isDefined(PAGE_SELLER.DATA.pagination) == true && isDefined(PAGE_SELLER.DATA.paginationEle) == true){
				//$infoEle.pagination('selectPage', curPage);
				//PAGE_SELLER.DATA.paginationEle.pagination('selectPage', curPage);
				//$infoEle.('selectPage', curPage);
			}else{
		...
		if(isPageShow == true){
			if(isDefined(PAGE_SELLER.DATA.pagination) == true && isDefined(PAGE_SELLER.DATA.paginationEle) == true){
				...
				console.log('pagingEleData', pagingEleData);
...
PAGE_SELLER.contents.callbackFun = function(data, saveData){
	...
	$( "#seller-contents-list-check-all" ).bind( "click", function() {
		...
		if($(this).prop("checked")) {
			$('#sellerContentsOwnerListForm').find("input[name=seller_contents_check]").prop("checked",true);
		}else{
			$('#sellerContentsOwnerListForm').find("input[name=seller_contents_check]").prop("checked",false);
		}
	});
...
//콘텐츠 삭제
PAGE_SELLER.contents.onclickDelContentsList = function(){
	console.log('PAGE_SELLER.contents.onclickDelContentsList');
...
//게시정보 변경
PAGE_SELLER.contents.onclickChangeContentsState = function(thisEle){
	console.log('PAGE_SELLER.contents.onclickChangeContentsState');
	...
	PAGE_SELLER.contents.actionContentsChange(eleData);

};

//게시중지
PAGE_SELLER.contents.actionContentsChange = function(eleData){
	console.log('PAGE_SELLER.contents.actionContentsChange');
	console.log('eleData', eleData);
	...
	var successFunChangeContents = function(data){
		...
		if(data.change_action == 'pulling'){
			if(isDefined(data.access_token)){
				alert('activex exc '+data.access_token);
			}
		}else{
...
PAGE_SELLER.info.edit.doneCallbackFun = function(data){
	...
	if(isDefined(data.seller)){
		...
		if(isDefined(data.seller.seller_profile_picture)){
			var timestamp = new Date().getTime();
			//$("#sellerProThumbTargetLayer").find('img.s_thumb').attr({'src':gData.seller_image+'?t='+timestamp});
			$('.seller-info-wrap').find('.seller_img.main_img img').attr({'src':data.seller.seller_profile_picture+'?t='+timestamp});
		}
	}
